...
5. **A candidate bug is a hypothesis — adversarially verify it.** Write a failing test that asserts the CORRECT (intent-derived) behavior; if it fails on unmutated code, the bug is real (the test documents it). Try to refute it (am I misreading the spec? is there a guard elsewhere?). Spawn independent skeptics for anything subtle. Then REPORT it (issue / PR description / surfaced finding) — distinct from the coverage work. Never silently "fix" by weakening the assertion to match the code.
