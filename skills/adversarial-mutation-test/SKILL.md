...
4. **Kill each surviving mutant** — step 2's survivor set is the worklist. Make a test catch it:
...
- **Probe the pre-existing suite BEFORE writing a test** — attribution exists only while the suite is untouched. Write first and every probe measures your own tests too; recovering which mutants the ORIGINAL tests killed then costs a second clone at the base commit, a second full mutation pass, and a diff of the two matrices.
