...
0. **Ingest authoritative oracles FIRST, and calibrate against known issues.** If a spec, NatSpec, audit report, or issue tracker documents intended behavior or KNOWN issues, that is the oracle — start there. Do NOT substitute a convenient intent the code happens to satisfy (the classic self-own: observing "dust is retained by the contract" and calling it "conservative, safe" when the audit calls the same behavior an orphaned-funds finding). When a labeled set of real issues exists (audit findings, tracker), the search MUST rediscover the still-open ones: reproduce each as a failing test against current code before trusting the detector on un-labeled areas. A search that cannot flag bugs you already KNOW exist is weak — fix the search, don't conclude the code is clean. Also widen the bar beyond "exploitable": accounting/UX/correctness divergences (orphaned funds, reverts-that-should-succeed, wrong-but-not-stolen values) are findings too.
1. **Derive the intended behavior independently** of the code: from the authoritative oracle above, plus type/unit constraints and domain invariants. Write the expected value/property from intent — do NOT read it off the code's current output. Sanity-check your derivation against any known finding in the same area so you don't analyze a real bug as safe.
...
3. **Try to FALSIFY each** — adversarially. Construct the inputs, sequences, boundaries, and hostile counterparties most likely to violate the property: extreme/zero/max values, non-default decimals, dust/rounding edges, repeated/reordered/interleaved operations, reentrant callbacks, unexpected token behaviors, self-referential parties. A single falsifying case is a **candidate bug**.
...
- **Isolate in a dedicated CLONE before the first mutation — your mutate→restore cycle is destructive to ANY concurrent uncommitted work in the same checkout, not just parallel workers'.** Mutation testing edits source and reverts it with `git checkout <paths>`, and artifact regen overwrites generated/etched files; in a checkout another session or human shares, that silently clobbers their **unstaged** changes, which are unrecoverable (no reflog for working-tree reverts). This is the SAME rule as the parallel fan-out below: work in a **fresh clone** (`git clone`), provision it (install deps + build), run the campaign, push the branch. NOT a `git worktree` — a worktree shares the original repo's object store and refs, so it is not hermetic; only a clone guarantees nothing you do (a stray `checkout`, `reset`, ref delete, regen) can reach the original checkout. (`git worktree list` tells you whether a checkout is already shared, e.g. multiple worktrees of one repo.)
