...
- **Model: Opus.** Run this skill's agents on **Opus 5** — pass `model:'opus'` in the `agent()` opts (the `opus` alias resolves to the current Opus generation, so this does not rot when the next one ships). Do NOT let the bug-finding half inherit a cheaper session model: finding a real bug means deriving intended behavior independently from the spec and constructing a falsifying case, which is exactly where model capability shows up. The mechanical probe stage may stay on the session default if cost demands it, but the adversarial pass, survivor-killing and synthesis run on Opus.
